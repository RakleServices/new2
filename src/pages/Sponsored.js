...
import { GetSponsored, InsertSponsored, DeleteSponsored } from '../api';
...
function Sponsored() {
    ...
    const deleteData = async id => {
        // alert(id);
        await DeleteSponsored(id).then(res => {
          if(res && res.data){
                                                ...
                                                <tbody>
                                                    {
                                                       ban !== "No Rows Found" ? ban.map((item) => (
                                                            < tr >
                                                                <td>{item.id}</td>
                                                                {/* <td>{item.banimage}</td> */}
                                                                <td><img height={"80px"} width={"80px"} src={"http://service.nilamiadda.com/spons/" + item.spnimage} alt="" /></td>
...
function DeleteCatagoryModal(props){
    ...
    return (
      ...
      >
        <Modal.Header >
          <Modal.Title>Delete Sponsored</Modal.Title>
        </Modal.Header>
        <Modal.Body>
        ...
        <Button variant="primary" onClick={delonHide}>Close</Button>
          <Button variant="primary" onClick={deleteDataForm}>Delete Sponsored</Button>
        </Modal.Footer>
      </Modal>
    )
}
function MyVerticallyCenteredModal(props){
    ...
    const submitForm =() =>{
        ...
        if(!selectedFile){
            ...
            formData.append("spnimage", selectedFile);
    ...
    return (
      ...
      >
        <Modal.Header >
          <Modal.Title>{bn && bn.catid ? 'Update' : 'Add'} Sponsored</Modal.Title>
        </Modal.Header>
        <Modal.Body>
           ...
           <div >
            <div className='col-md-12'>
                ...
                <Form.Group className="mb-3" controlId="formBasicPassword">
                    <Form.Label>Sponsored Image</Form.Label>
        ...
        <Button variant="primary" onClick={onHide}>Close</Button>
          <Button variant="primary" onClick={submitForm}>{bn && bn.catid ? 'Update' : 'Add'} Sponsored</Button>
        </Modal.Footer>
      </Modal>
    )
}